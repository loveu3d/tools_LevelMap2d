...
public class SpriteAction : MonoBehaviour {
	...
	void Start () {
		...
		{
				float x_value = this.GetComponent<MeshFilter>().mesh.bounds.size.x;
				float y_value = this.GetComponent<MeshFilter>().mesh.bounds.size.y;
				Debug.Log("x:"+x_value+"  y:"+y_value);

			copyWidth=this.GetComponent<Renderer>().material.mainTexture.width;
